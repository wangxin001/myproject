            ...
            try
            {
                ...
                if (oFile != null)
                {
                    ...
                    if (Connector.CheckFileName(sFileName) && !Config.Current.CheckIsHiddenFile(sFileName))
                    {
                        ...
                        while (true)
                        {
                            String sGuidFileName = Guid.NewGuid() + System.IO.Path.GetExtension(oFile.FileName);

                            /**
                             * 将上传的文件名保存为不重名的GUID文件名形式。
                             * 
                             * 2010-12-10 张洲峰
                             */
                            // sFilePath = System.IO.Path.Combine( sServerDir, sFileName );
                            sFilePath = System.IO.Path.Combine(sServerDir, sGuidFileName);

                            if (System.IO.File.Exists(sFilePath))
                            {
                                iCounter++;
                                //sFileName = sFileNameNoExt + "(" + iCounter + ")" + System.IO.Path.GetExtension(oFile.FileName);
                                //sGuidFileName = Guid.NewGuid() + System.IO.Path.GetExtension(oFile.FileName);
